...
import { sendMessage, expireChats } from "@/lib/server/chats";

/** Shares a MEMORE post into the chat as a reference (post_id) — the meme
 * itself is never duplicated. The recipient can invest in it in place. Any
 * sender note arrives pre-encrypted in `ciphertext`. */
